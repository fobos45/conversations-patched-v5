...
public class ConversationAdapter
    ...
    private static final int[] SERVER_COLORS = {
        0x1A2196F3, 0x1A4CAF50, 0x1AF44336, 0x1AFF9800,
        0x1A9C27B0, 0x1A00BCD4, 0x1AFF5722, 0x1A607D8B
    };
    ...
    private int getServerColor(final String server) {
        if (!serverColorMap.containsKey(server)) {
            serverColorMap.put(server, SERVER_COLORS[serverColorMap.size() % SERVER_COLORS.length]);
        }
    ...
    public void onBindViewHolder(@NonNull ConversationViewHolder viewHolder, int position) {
        ...
        if (conversation == ConversationFragment.getConversation(activity)) {
            ...
            final int serverColor = getServerColor(server);
            viewHolder.binding.frame.setBackgroundColor(
                    Color.argb(
                            Color.alpha(serverColor),
                            Color.red(serverColor),
                            Color.green(serverColor),
                            Color.blue(serverColor)));
        }
